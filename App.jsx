...
const percentage = Math.min(Math.round((earnedCredits / required) * 100), 100);
    ...
    return {
      ...
      isComplete: earnedCredits >= required,
    ...
    const printContent = `
      ...
      <body>
        <div class="header">
          ...
          <p>ScholarPath Graduation Progress Student Credit Record</p>
        </div>
        ...
        <div class="footer">
          <p>Generated by ScholarPath Graduation Progress • ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
        </div>
      </body>
      </html>
    `;
...
function AuthScreen({ onLogin }) {
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="relative w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">ScholarPath Graduation Progress</h1>
  ...
  disabled={loading}
  className="w-full mb-4 flex items-center justify-center gap-3 bg-white hover:bg-gray-100 text-gray-800 font-medium py-3 px-4 rounded-xl transition-all disabled:opacity-50"
>
...
{showLinkParentModal && (
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div className="bg-slate-900 rounded-3xl w-full max-w-md border border-slate-700 p-6 max-h-[90vh] overflow-y-auto">
                ...
                {allStudents.map(student => (
                  <button
                    ...
                    onClick={async () => {
                      ...
                      if (!error) { alert('Parent linked successfully!'); setShowLinkParentModal(false); }
      ...
      {showMappingsModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 rounded-3xl w-full max-w-2xl border border-slate-700 p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-white">📚 Course Mappings</h2>
              <button onClick={() => setShowMappingsModal(false)} className="text-slate-400 hover:text-white p-2">✕</button>
            </div>
            
            <p className="text-slate-400 text-sm mb-4">Map course names from Engage to credit categories and amounts.</p>
            ...
            <form onSubmit={async (e) => {
              ...
              const credits = parseFloat(form.credits.value) || 0.5;
              if (!courseName) return;
              const { error } = await supabase.from('course_mappings').insert([{ school_id: profile.school_id, course_name: courseName, category_id: categoryId || null, credits }]);
              if (!error) {
                const { data } = await supabase.from('course_mappings').select('*').eq('school_id', profile.school_id);
                setCourseMappings(data || []);
                form.reset();
              } else { alert('Error: ' + error.message); }
            }} className="bg-slate-800/50 rounded-xl p-4 mb-4">
              <div className="grid grid-cols-3 gap-3">
                <input name="courseName" placeholder="Course name (e.g. Algebra IA)" className="col-span-3 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm" required />
                <select name="categoryId" className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm">
                  <option value="">Select Category</option>
                  {categories.map(cat => (<option key={cat.id} value={cat.id}>{cat.name}</option>))}
                </select>
                <input name="credits" type="number" step="0.25" defaultValue="0.5" className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm" />
                <button type="submit" className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg px-3 py-2 text-sm">Add</button>
              </div>
            </form>
            
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {courseMappings.length === 0 ? (
                <p className="text-slate-500 text-center py-4">No course mappings yet.</p>
              ) : (
                courseMappings.map(mapping => {
                  const cat = categories.find(c => c.id === mapping.category_id);
                  return (
                    <div key={mapping.id} className="flex items-center justify-between bg-slate-800/50 rounded-lg p-3">
                      <div>
                        <p className="text-white font-medium">{mapping.course_name}</p>
                        <p className="text-slate-400 text-sm">{cat?.name || 'No category'} • {mapping.credits} credits</p>
                      </div>
                      <button onClick={async () => {
                        await supabase.from('course_mappings').delete().eq('id', mapping.id);
                        setCourseMappings(courseMappings.filter(m => m.id !== mapping.id));
                      }} className="text-red-400 hover:text-red-300 p-2">🗑</button>
                    </div>
                  );
                })
              )}
            ...
            <button onClick={() => setShowMappingsModal(false)} className="w-full mt-6 bg-slate-800 text-slate-300 font-medium py-3 rounded-xl hover:bg-slate-700">Close</button>
          </div>
        </div>
      )}
    </div>
...
advisingNotes.slice(0, 5)
    ...
    const categoryProgress = (student.stats?.categories || categories).map(cat => {
    ...
    const printContent = `
      ...
      <body>
        <div class="header">
          <h1>📋 Advising Plan</h1>
          <p>Summit Learning Charter &nbsp;|&nbsp; ${today} &nbsp;|&nbsp; ScholarPath Graduation Progress</p>
        </div>
        ...
        <div class="footer">
          <p>ScholarPath Graduation Progress — Advising Plan — ${student.full_name} — Generated ${today}</p>
...
{student.has_iep && (
  ...
  <button 
    onClick={async () => {
                  ...
                  {/* Tabs + action buttons — all same visual size, wraps on narrow screens */}
                  <div className="flex flex-wrap items-center gap-2 mt-4">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-xl text-sm font-medium transition-all bg-indigo-500 hover:bg-indigo-600 text-white"
                    >
                      ➕ Add Course
                    </button>
                    <button
                      onClick={() => setShowInviteParentModal(true)}
                      className="px-4 py-2 rounded-xl text-sm font-medium transition-all bg-purple-500 hover:bg-purple-600 text-white"
                    >
                      ✉️ Invite Parent
                    </button>
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${student.is_active === false ? 'bg-green-500 hover:bg-green-600 text-white' : 'bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 border border-amber-500/30'}`}
                    >
...
{showInviteParentModal && (
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div className="bg-slate-900 rounded-3xl w-full max-w-md border border-slate-700 p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-white">✉️ Invite Parent</h2>
        <button
          onClick={() => {
            setShowInviteParentModal(false);
            setInviteParentEmail('');
            setInviteParentError('');
            setInviteParentSuccess(false);
          }}
      ...
      {inviteParentSuccess ? (
        <div className="text-center py-6">
          ...
          <p className="text-emerald-400 font-semibold text-lg">Invite sent!</p>
          <p className="text-slate-400 text-sm mt-2">
            {inviteParentEmail} will receive an email to create their ScholarPath Graduation Progress parent account.
            They'll automatically be linked to {student.full_name} when they sign up.
          </p>
          <button
            onClick={() => {
              setShowInviteParentModal(false);
            ...
            className="mt-6 bg-slate-800 text-slate-300 px-6 py-2 rounded-xl hover:bg-slate-700 transition-all"
          >
            Close
          </button>
        </div>
        ...
        <>
          <p className="text-slate-400 text-sm mb-4">
            Enter the parent's email address. They'll receive an invitation to create a ScholarPath Graduation Progress account
            and will automatically be linked to <span className="text-white font-medium">{student.full_name}</span>.
          </p>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={e => {
                  setInviteParentEmail(e.target.value);
                  setInviteParentError('');
                }}
            ...
            {inviteParentError && (
              <p className="text-red-400 text-sm">{inviteParentError}</p>
            )}
          </div>
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button
              onClick={async () => {
                if (!inviteParentEmail.trim() || !inviteParentEmail.includes('@')) {
                  setInviteParentError('Please enter a valid email address.');
                  return;
                }
                setInviteParentSending(true);
                setInviteParentError('');
                try {
