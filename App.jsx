...
export default function App() {
  ...
  useEffect(() => {
   ...
   // ============================================
  // IDLE SESSION RECOVERY — Feb 14, 2026
  // Re-validates auth when user returns to tab
  // Fixes: notes timeout after leaving app idle
  // ============================================
  useEffect(() => {
    ...
    const handleVisibilityChange = async () => {
      ...
      try {
        console.log('Tab visible — checking session freshness');
        const { data: { session }, error } = await supabase.auth.getSession();

        if (error || !session) {
          console.warn('Session expired during idle — logging out');
          setUser(null);
          setProfile(null);
          localStorage.clear();
        ...
        if (timeLeft < 300) {
          ...
          console.log(`Session valid — ${Math.round(timeLeft / 60)} min remaining`);
        }
      } catch (err) {
  ...
  // ============================================
  // SESSION KEEP-ALIVE — Feb 14, 2026
  // Periodic check prevents zombie sessions
  // ============================================
  useEffect(() => {
    if (!user) return;
    ...
    const keepAlive = setInterval(async () => {
      try {
        ...
        if (error || !session) {
          console.warn('Keep-alive: session expired — logging out');
        ...
        if (timeLeft < 600) {
          console.log('Keep-alive: proactive token refresh');
          await supabase.auth.refreshSession();
        }
      } catch (err) {
        console.warn('Keep-alive check failed (network?):', err.message);
      }
    }, 4 * 60 * 1000);
