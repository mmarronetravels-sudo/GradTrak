        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 rounded-3xl w-full max-w-2xl border border-slate-700 p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowMappingsModal(false)} className="text-slate-400 hover:text-white p-2">
                <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
            ...
            <form onSubmit={async (e) => {
              ...
              const { error } = await supabase.from('course_mappings').insert([{
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <input name="credits" type="number" step="0.25" defaultValue="0.5" placeholder="Credits" className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-white text-sm" />
            ...
            <button onClick={() => setShowMappingsModal(false)} className="w-full mt-6 bg-slate-800 text-slate-300 font-medium py-3 rounded-xl hover:bg-slate-700 transition-all">
              Close
            </button>
          </div>
        </div>
