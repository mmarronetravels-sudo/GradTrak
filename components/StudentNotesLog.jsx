...
const NoteEntry = ({ note, counselorId, onStatusToggle, onDelete, onEdit, editState }) => {
  ...
  : formatDate(note.contact_date || note.created_at)}
        ...
        {/* Edit button (always visible, author only) */}
        {isAuthor && (
          <button
            onClick={() => onEdit(note)}
            className="absolute top-2 right-10 p-1.5 rounded text-slate-500 hover:text-cyan-400 hover:bg-cyan-500/10 transition-all"
            title="Edit note"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const generateMTSSReport = (notes, studentName, studentGrade, counselorName) => {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(
      `Page ${i} of ${totalPages} | Generated by ScholarPath Graduation Progress | ${new Date().toLocaleDateString()}`,
      pageWidth / 2,
      pageHeight - 10,
      { align: 'center' }
    );
